...
func _on_T_pressed():
	if selected_btn:
		if selected_btn != $container/T:
			$container/T.self_modulate = selected_btn.self_modulate
			selected_btn.self_modulate = Color8(255,255,255,255)
			selected_btn = $container/T
...
func _on_CT_pressed():
	if selected_btn:
		if selected_btn != $container/CT:
			$container/CT.self_modulate = selected_btn.self_modulate
			selected_btn.self_modulate = Color8(255,255,255,255)
			selected_btn = $container/CT
